// BitMask: 이진수를 사용하는 컴퓨터의 연산 방식을 이용, 정수의 이진수 표현을 자료 구조로 활용하는 기법
// 이진수는 0 or 1을 사용하므로 하나의 Bit가 표현할 수 있는 경우는 2가지.
// 1이면 켜져 있다, 0이면 꺼져 있다.

// 장점
// 1. 수행 시간이 빠르다: bit 연산이므로 시간복잡도가 O(1) 인 경우가 많다. 비트마스크를 이용하는 경우에는 비트의 개수만큼 원소를 다룰 수 있기 때문에 연산 횟수가 늘어날수록 차이가 커진다.
// 2. 코드가 짧다: 집합 연산을 비트연산자로 작성하므로 훨씬 간결하다.
// 3. 메모리 사용량이 적다: bit가 n개라면 각 bit 당 2가지 경우를 가지므로 2^n 가지의 경우를 10bit 이진수 하나로 표현 가능하다. 이처럼 하나의 정수로 매우 많은 경우의 수를 표현할 수 있으므로 메모리 측면에서 효율적이며, 더 많은 데이터를 미리 계산해서 저장해 둘 수 있다(DP에 유용하다).
...
// 따라서 비트 연산자를 사용할 경우 연산자마다 괄호를 씌워주는 것이 바람직하다.
// 2. 오버플로우
// 2의 50승을 구하기 위해 1 << 50으로 표현하면, C에서 1이 32bit 상수 취급되므로, 왼쪽으로 50번 shift하면 오버플로우가 발생한다. 따라서 1LL로 표현해야 한다.

// 비트마스크를 이용한 집합 구현은 가장 대표적이다. 하나의 bit가 하나의 원소가 되는 것이다. bit가 켜져 있으면 해당 원소가 집합에 포함되어 있다는 의미이다.
// 따라서 N비트 정수 변수라면 N개의 원소를 갖는 집합의 부분집합들을 모두 표현할 수 있게 된다.
// 원래 N개의 boolean 원소를 갖는 배열을 선언해야 했지만, 비트마스킹을 이용하면 정수 하나로 표현이 가능하다.
